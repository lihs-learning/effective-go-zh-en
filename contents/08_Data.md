...
复合字面量的字段必须按顺序全部列出。但如果以 `field: value` 的形式明确地标出元素，初始化字段时就可以按任何顺序出现，未给出的字段值将赋予零值。因此，我们可以用如下形式：
...
### Allocation with make 使用 make 分配内存
...
Go 采用的格式化打印风格和 C 的 `printf` 系列类似，但却更加丰富而通用。这些函数位于 fmt 包中，且函数名首字母均为大写：如 `fmt.Printf`、`fmt.Fprintf`，`fmt.Sprintf` 等。字符串函数（`Sprintf` 等）会返回一个字符串，而非填充给定的缓冲区。
...
另一种实用的格式是 %T，它会打印某个值的类型。`fmt.Printf("%T\n", timeZone)` 会打印
...
若你想控制自定义类型的默认格式，只需为该类型定义一个签名为 `String() string` 的方法。对于我们简单的类型 T，可进行如下操作。
...
我们的 String 方法也可调用 `Sprintf`，因为打印例程可以完全重入并按这种方式封装。不过要理解这种方式，还有一个重要的细节：请勿通过调用 `Sprintf` 来构造 String 方法，因为它会无限递归你的 String 方法。当 `Sprintf` 试图直接以字符串的形式打印接受者时，这种情况就会出现，因为在此过程中又会反过来调用 String 方法。这是一个很常见的错误，如下例所示。
