...
```go
go list.Sort()  // run list.Sort concurrently; don't wait for it. // 并发运行 list.Sort，无需等它结束。
```
...
<-c   // Wait for sort to finish; discard sent value. // 等待排序结束，丢弃发来的值。
```
...
func handle(r *Request) {
	sem <- 1    // Wait for active queue to drain.   // 等待活动队列清空。
	process(r)  // May take a long time.             // 可能需要很长时间。
	<-sem       // Done; enable next request to run. // 完成；使下一个请求可以运行。
}

func Serve(queue chan *Request) {
	for {
		req := <-queue
		go handle(req)  // Don't wait for handle to finish. // 无需等待 handle 结束。
	}
}
```
...
func Serve(queue chan *Request) {
	for req := range queue {
		sem <- 1
		go func() {
			process(req) // Buggy; see explanation below. // 这儿有 Bug，解释见下。
			<-sem
		}()
	}
}
...
func Serve(queue chan *Request) {
	for req := range queue {
		req := req // Create new instance of req for the goroutine. // 为该 Go 程创建 req 的新实例。
		sem <- 1
		go func() {
			process(req)
			<-sem
...
func Serve(clientRequests chan *Request, quit chan bool) {
	...
	<-quit // Wait to be told to exit. // 等待通知退出。
}
```
...
func (v Vector) DoSome(i, n int, u Vector, c chan int) {
	...
	c <- 1 // signal that this piece is done // 发信号表示这一块计算完成。
}
```
...
func (v Vector) DoAll(u Vector) {
	c := make(chan int, NCPU) // Buffering optional but sensible. // 缓冲区是可选的，但明显用上更好
	for i := 0; i < NCPU; i++ {
...
func client() {
	for {
		...
		// 抢占一个可用的缓冲区，若没有可用的缓冲区，则分配一个新的缓冲区。
		select {
		case b = <-freeList:
		...
		default:
			...
			// 没有可用的缓冲区，所以分配一个新的缓冲区。
			b = new(Buffer)
		}
		load(b)         // Read next message from the net. // 从网络中读取下一条消息。
		serverChan <- b // Send to server.                 // 发送至服务器。
	}
}
```
