...
在 Go 的库中，你会发现若 if 语句不会执行到下一条语句时，亦即其执行体以 break、continue、goto 或 return 结束时，不必要的 else 会被省略。
...
### Redeclaration and Reassignment 重新声明与再次赋值
...
对于字符串，range 能够提供更多便利。它能通过解析 UTF-8，将每个独立的 Unicode 码点分离出来。错误的编码将占用一个字节，并以字符（rune）U+FFFD 来代替。（内建类型 rune 是 Go 对单个 Unicode 码点的称谓。详情见语言规范）。循环
...
switch 并不会自动向下执行其他 case，但 case 可通过逗号分隔来列举相同的处理条件。
...
switch 也可用于判断接口变量的动态类型。例如通过圆括号包裹 type 关键字使用类型断言语法。若 switch 在表达式中声明了一个变量，那么该变量的每个子句中都将有该变量对应的类型。在这些 case 中重用一个名字也是符合语义的，实际上是在每个 case 里声明了一个不同类型但同名的新变量。
