...
// 
// PathError 记录一个错误以及产生该错误的文件路径和操作。
type PathError struct {
	Op string    // "open", "unlink", etc.       // "open"、"unlink" 等等。
	Path string  // The associated file.         // 相关联的文件。
...
for try := 0; try < 2; try++ {
	...
	if e, ok := err.(*os.PathError); ok && e.Err == syscall.ENOSPC {
		deleteTempFiles() // Recover some space. // 清理出一些空间。
		continue
	}
	return
}
...
func CubeRoot(x float64) float64 {
	z := x/3 // Arbitrary initial value // 随意设置的初始值
	for i := 0; i < 1e6; i++ {
		prevz := z
		z -= (z*z*z-x) / (3*z*z)
...
// error 是 *Regexp 的一个方法，它通过一个 Error 触发 Panic 来报告解析错误。
func (regexp *Regexp) error(err string) {
	panic(Error(err))
}

// Compile returns a parsed representation of the regular expression.
// Compile 返回一个被解析后的正则表达式。
func Compile(str string) (regexp *Regexp, err error) {
	regexp = new(Regexp)
	// doParse will panic if there is a parse error.
	// 如果出现一个解析错误，doParse 将会导致 panic
	defer func() {
		if e := recover(); e != nil {
			regexp = nil    // Clear return value.                 // 清理返回值
			err = e.(Error) // Will re-panic if not a parse error. // 若不是一个解析错误，将重新触发 Panic。
		}
	}()
